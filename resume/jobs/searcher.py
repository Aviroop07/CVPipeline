...
from resume.linkedin.fetcher import authenticate_linkedin
...
def load_job_roles() -> List[str]:
    ...
    """
    job_roles_file = config.PROJECT_ROOT / config.ASSETS_DIR / config.JOB_ROLES_FILE
...
def save_job_results(jobs: List[Dict[str, Any]], output_path: pathlib.Path = None, location: str = None, jobs_per_role: int = None) -> pathlib.Path:
    ...
    if output_path is None:
        output_path = config.PROJECT_ROOT / config.ASSETS_DIR / config.JOB_SEARCH_RESULTS_FILE
...
def search_and_save_jobs(location: str = None, jobs_per_role: int = None) -> pathlib.Path:
    ...
    if not jobs:
        log.warning("?????? No jobs found - creating empty results file")
        jobs = []
...
def clear_job_search_cache(expired_only: bool = True) -> int:
    ...
    if not config.API_CACHE_ENABLED:
        log.info("?????? API caching is disabled")
        return 0
